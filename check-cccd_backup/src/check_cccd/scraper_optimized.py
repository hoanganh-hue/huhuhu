"""
Optimized Scraper Module for Check CCCD
...
from .cache_layer import get_cached_cccd_result, cache_cccd_result
...
def scrape_cccd_optimized(cccd: str) -> Dict:
    """Optimized synchronous fetch + parse flow for a single CCCD with caching and connection pooling.
    ...
    try:
        ...
        if cached_result:
            logger.info("Cache hit for CCCD", cccd=cccd, cached_at=cached_result.get('cached_at'))
...
def _scrape_with_retry_optimized(cccd: str) -> Dict:
    """Scrape with retry logic, connection pooling and anti-bot strategies."""
    last_error = None
    request_strategy = RequestStrategy()

    for attempt in range(MAX_RETRIES):
        try:
            return _scrape_single_attempt_optimized(cccd, attempt, request_strategy)
        except Exception as e:
            last_error = e
...
def _scrape_single_attempt_optimized(cccd: str, attempt: int, request_strategy: RequestStrategy) -> Dict:
    """Single scraping attempt with connection pooling and optimized strategies."""
    ...
    try:
        ...
        homepage_response = client.get(homepage_url, timeout=REQUEST_TIMEOUT)
    ...
    try:
        search_page_response = client.get(search_page_url, timeout=REQUEST_TIMEOUT)
    ...
    for i, match in enumerate(matches):
        ...
        try:
            ...
            time.sleep(1.0)
            detailed_match = _fetch_profile_details_optimized(match, client)
...
def _fetch_profile_details_optimized(match: Dict, client: httpx.Client) -> Dict:
    """Fetch detailed information from profile page using connection pooling."""
    url = match.get("url")
    if not url:
        return match
...
def _extract_address_from_profile(soup: BeautifulSoup) -> Optional[str]:
    """Extract address from profile page with enhanced patterns."""
    # Look for address-related text with comprehensive patterns
    address_patterns = [
        r'Địa chỉ[:\s]*(.+?)(?:\n|$)',
    ...
    # Try to find address by looking for geographic indicators
    geo_patterns = [
        r'(?:Thành phố|Quận|Huyện|Phường|Xã|Số \d+|Đường|Tỉnh).+?(?:,|\n|$)',
        r'\d{1,3}[/-]\d{1,3}[/-]\d{1,3}.+?(?:,|\n|$)',  # Street numbers
        r'(?:Hà Nội|TP\.? Hồ Chí Minh|Sài Gòn).+?(?:,|\n|$)'
    ]

    for pattern in geo_patterns:
        matches = re.findall(pattern, text, re.IGNORECASE)
        for match in matches:
            if len(match.strip()) > 15:  # Longer addresses are more likely to be complete
                return match.strip()
...
def _extract_role_from_profile(soup: BeautifulSoup) -> Optional[str]:
    """Extract role/position from profile page with enhanced patterns."""
    role_patterns = [
        r'Chức vụ[:\s]*(.+?)(?:\n|$)',
        r'Position[:\s]*(.+?)(?:\n|$)',
        r'Role[:\s]*(.+?)(?:\n|$)',
