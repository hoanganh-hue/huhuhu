...
def _parse_search_results(soup: BeautifulSoup, cccd: str) -> List[Dict]:
    """Parse search results page with improved selectors - FIXED VERSION."""
    matches = []

    # Look for actual company profile links (exclude ads and navigation)
    profile_selectors = [
        'a[href*="/"]',  # Any link
        '.search-result a',
        '.result-item a',
    ...
    for selector in profile_selectors:
        ...
        for link in links:
            ...
            if _is_navigation_or_ad_link(href):
                continue

            # Check if this looks like a real company profile link
            if _is_company_profile_link(href):
                match = {
                ...
                # Don't break - collect all valid company links
...
def _is_navigation_or_ad_link(href: str) -> bool:
    """Check if href is a navigation link or advertisement."""
    if not href:
        return False
    ...
    nav_patterns = [
        ...
        r'^#',   # Any anchor link
        r'^/$',  # Root path
        r'/tra-cuu-ma-so-thue',  # Search pages
        r'/Search',  # Search endpoint
        r'facebook\.com',
        r'twitter\.com',
        r'youtube\.com',
        r'instagram\.com'
    ...
    # Check for Zalo links (advertisements)
    if 'zalo.me' in href:
        return True
...
def _is_company_profile_link(href: str) -> bool:
    """Check if href is a real company profile link (not ad or nav)."""
    if not href:
        return False

    # Must be a profile link AND not navigation/ad
    return _is_profile_link(href) and not _is_navigation_or_ad_link(href)
